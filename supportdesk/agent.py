"""Support Bot: a small A2A agent that reaches its tools over MCP.

Adapted from the MiDojo weather example agent (Apache-2.0,
https://github.com/asago-ai/midojo). Any OpenAI-compatible endpoint works:
vLLM, a Red Hat OpenShift AI MaaS route, or Ollama.
"""
...
from supportdesk import SYSTEM_MESSAGE
...
support_skill = AgentSkill(
    id="support_desk",
    name="Support Desk",
    description="Look up customers and orders, send follow-up emails",
    tags=["support"],
    examples=["What's the status of order 4521?"],
)
...
@click.option(
    ...
    help="Base URL advertised in the agent card. Set it when the agent is reached from another host, "
    "e.g. a Kubernetes Service DNS name. Defaults to http://localhost:<port>/.",
)
...
def main(
    ...
    executor = SupportAgentExecutor(llm, litellm_model, mcp_server_url)

    agent_card = AgentCard(
        name="Support Bot",
        description="A customer support assistant that uses MCP tools.",
        version="1.0.0",
        supported_interfaces=[
            AgentInterface(url=public_url or f"http://localhost:{port}/", protocol_binding="JSONRPC"),
        ],
    ...
    print(f"Starting Support Bot (A2A) on port {port}")
